...
class BlogDao():
    ...
    def search_blogs_with_limit(self, keyword, limit):
        # 首先获取到数据库的链接类
        connection = Connetion()
        # 获取到链接
        conn = connection.get_connetction()
        ...
        try:
            print "SELECT * from blog WHERE {0} order BY post_time".format(condition)
            cursor.execute("SELECT * from blog WHERE {0} order BY post_time limit {1}".format(condition, limit))
    ...
    def search_all_blogs_without_keyword(self):
        # 首先获取到数据库的链接类
        connection = Connetion()
        # 获取到链接
        conn = connection.get_connetction()
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT * from blog order BY post_time")
            conn.commit()
        except:
            conn.rollback()
        # 访问结束关闭链接
